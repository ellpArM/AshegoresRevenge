...
public class RedMageMeteor : BaseSkill
{
    ...
    public override IEnumerator Execute()
    {
        ...
        yield return BattleManagerNew.Instance.StartCoroutine(WaitForTargetAndCastMeteor());
    ...
    private IEnumerator PerformMeteorVisuals(FightingEntity target)
    {
    ...
    private IEnumerator MoveElementalEffectsToTargetArea(FightingEntity target, float duration)
    {
        if (elementalEffects == null)
            elementalEffects = new List<GameObject>();
        else
            elementalEffects.Clear();

        for (int i = 0; i < 5; i++)
            elementalEffects.Add(Instantiate(groundEffectPrefab, transform.position, Quaternion.identity));
    ...
    private IEnumerator ApplyAreaDamage(FightingEntity mainTarget)
    {
